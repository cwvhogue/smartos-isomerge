...
function chown(options, callback)
{
  ...
  log(' * #cyan[chown ' + options.owner + ':' + options.group + '] ' +
    options.file);
  var args = [ options.owner + ':' + options.group, options.file ];
  execFile('/bin/chown', args, function(err, stdout, stderr) {
    if (err) return callback(err);
    return callback();
  });
}
...
{
  assert(options.file);
  assert(options.perms);
  log(' * #cyan[chmod ' + options.perms + '] ' + options.file);
  var args = [ options.perms, options.file ];
  execFile('/bin/chmod', args, function(err, stdout, stderr) {
    if (err) return callback(err);
    return callback();
  });
}

function mkdir_p(options, callback)
{
  assert(options.path);
  log(' * #cyan[mkdir] ' + options.path);
...
function ln(options, callback)
{
  assert(options.dest);
  assert(options.target);
  log(' * #cyan[ln target] ' + options.target);
  log('   #cyan[     dest] ' + options.dest);
  var args = [options.dest, options.target];
  if (options.symbolic)
    args.unshift('-s');
  if (options.force)
    args.unshift('-f');
  execFile('/bin/ln', args, function(err, stdout, stderr) {
    if (err) return callback(err);
    return callback(null);
  });
}
